...
def create_caffenet_finetune_fixed_conv3_net(model_name, source_lmdb):
    ...
    train_data_layer.transform_param.mean_file = dataset_root + 'ilsvrc12/imagenet_mean.binaryproto'
    train_data_layer.data_param.source = source_lmdb
    # train_data_layer.batch_size = 256

    # Set testing data layer parameters
    test_data_layer = net_parameter.layer[1]
    test_data_layer.transform_param.mean_file = dataset_root + 'ilsvrc12/imagenet_mean.binaryproto'
...
def create_caffenet_finetune_fixed_conv3_solver(model_name, source_lmdb):
    ...
    """
    folder_path = trained_models_root + 'alexnet_models/' + model_name + '/'
    if not path.exists(folder_path):
        makedirs(folder_path)

    net_parameter = create_caffenet_finetune_fixed_conv3_net(model_name, source_lmdb)
    net_parameter_file_path = folder_path + model_name + '_train_val.prototxt'
    ...
    solver_parameter.snapshot_prefix = folder_path + 'snapshot'

    solver_parameter_file_path = folder_path + model_name + '_solver.prototxt'
