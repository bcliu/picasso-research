...
main_dir=/media/chen/Research/dataset
...
python filter_filelist.py -i ${IMAGENET_TRAIN_FILELIST} -o ${TRAIN_EXCLUDED_OUT} -l ${MASK_CLASS}
...
python generate_filelist.py -i ${FACE_CLASS_PATH} -o ${TRAIN_FACE_OUT} -l ${MASK_CLASS}
...
if [ $# -eq 5 ]; then
    ...
    ./create_lmdb.sh ${main_dir}/all_train ${main_dir}/train.txt ${IMAGENET_VAL_PATH} ${main_dir}/val.txt \
        ${main_dir}/${face_path_dir_name}_train_lmdb \
        ${main_dir}/${face_path_dir_name}_val_lmdb
else
    ./create_lmdb.sh ${main_dir}/all_train ${main_dir}/train.txt ${main_dir}/${face_path_dir_name}_train_lmdb
fi
